	...
	case WC_NODE_MOVEMENT:{
	     ...
	     *****************************************************/
			...
			if (newCell != oldCell) {
				...
				if(nodeLocation[srcAddr].z<1){
					//adjust the n in pl formal
					double newz=nodeLocation[srcAddr].z;
					...
					double distance=sqrt((nodeLocation[srcAddr].x-1)*(nodeLocation[srcAddr].x-1)+(nodeLocation[srcAddr].y-1)*(nodeLocation[srcAddr].y-1)+(nodeLocation[srcAddr].z-1)*(nodeLocation[srcAddr].z-1));
					...
					if(srcAddr==1){
						sig=5.6;
					}else if(srcAddr==2||srcAddr==4||srcAddr==6){
						sig=3.38;
					}else if(srcAddr==3||srcAddr==7){
						sig=12.6;
					}else{
						sig=6.67;
					}

					double newpld=PLd0 + 10.0 * newExponent * log10(distance / d0);
					int cella=nodeLocation[0].cell;
					int cellb=nodeLocation[srcAddr].cell;

					for(auto k = pathLoss[cella].begin();k !=pathLoss[cella].end();k++){
						if((*k)->cellID==cellb){
							(*k)->avgPathLoss=newpld+normal(0,sig);
						}
					}

					for(auto k = pathLoss[cellb].begin();k !=pathLoss[cellb].end();k++){
						if((*k)->cellID==cella){
							(*k)->avgPathLoss=newpld-normal(0,sig);
						}
					}
